...
with open('/home/lazaro/Workspace/wala-master/src/pages/Tienda/admin/VisualEditorPanel.jsx', 'r') as f:
    content = f.read()

# Define BackgroundStylesControl component
bg_control = """const BackgroundStylesControl = ({ settings, onChange }) => {
  return (
...
# Insert component before VisualEditorPanel
content = content.replace('const VisualEditorPanel = () => {', bg_control + '\nconst VisualEditorPanel = () => {')

# Regex to find and replace the "Estilos Adicionales" blocks
pattern = re.compile(r"<h5 style={{marginTop: '15px', marginBottom: '10px'}}>Estilos Adicionales</h5>.*?</div>\s*</div>\s*</div>", re.DOTALL)

replacement = """<BackgroundStylesControl 
              settings={s} 
              onChange={(key, value) => {
...
with open('/home/lazaro/Workspace/wala-master/src/pages/Tienda/admin/VisualEditorPanel.jsx', 'w') as f:
    f.write(new_content)
