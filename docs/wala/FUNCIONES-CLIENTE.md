...
- **Modo Noche — tema claro / oscuro / sistema (sesión 2026-06-29, ✅ desplegado):** un **interruptor luna/sol** en el Header (`src/components/common/ThemeToggle/`) deja que el visitante elija **claro**, **oscuro** o **seguir al sistema** (default: sigue el `prefers-color-scheme` del navegador/SO y reacciona en vivo si el visitante cambia el modo de su sistema). La elección se **recuerda** en `localStorage` (clave `wala-theme`) y se aplica a **toda la web** (tienda, cuenta, admin) vía `data-theme` en `<html>`; un **script anti-parpadeo** en `index.html` la fija **antes** de cargar para que la página no "salte" del claro al oscuro. Respeta los **colores de fondo que el admin configure** en cada sección (no los pisa) y garantiza **contraste** (fondo oscuro → texto claro). Motor: `src/contexts/ThemeContext.jsx`; paleta y overrides en `src/styles/globals.css` + ~40 `.module.css`. **No afecta carrito, precios, pagos ni datos.** Detalle en [MODO-NOCHE.md](./MODO-NOCHE.md).
