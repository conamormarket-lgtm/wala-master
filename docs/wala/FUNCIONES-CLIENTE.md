...
- **Qué hace (pago — diferenciado por país):**
  ...
  - **Confirmación de pago en backend (endurecida — SEGURO POR DEFECTO, sesión 2026-06-28):** el pago con Culqi se confirma server-side (recálculo del monto en `processCulqiPayment`, con **idempotencia por `tokenId`** mediante el lock `culqiCharges`) y la Cloud Function `culqiWebhook` marca el pedido (`pedidos_web`) como pagado de forma **idempotente**. La verificación de firma del webhook y la verificación de propiedad de la orden vienen detrás de flags **apagados por defecto** (`CULQI_VERIFY_SIGNATURE`, `ENFORCE_PAYMENT_OWNERSHIP`), así que desplegar el backend no cambia el cobro actual; ver el catálogo y los flags en [FUNCIONES-ADMIN.md → "Cloud Functions (backend)"](./FUNCIONES-ADMIN.md). Para **PayPal** existe además un flujo **server-side seguro** (`createPaypalOrderSecure` / `capturePaypalOrderSecure`) que recalcula el USD y valida la captura en el servidor; el cliente lo usa solo si el flag de build `VITE_PAYPAL_SERVER_SIDE` está en `'true'` (default OFF = flujo PayPal cliente histórico). *(Nota operativa: la URL de `culqiWebhook` debe estar registrada en el panel de Culqi y `REACT_APP_PAYPAL_CLIENT_ID` debe existir en Vercel para que PayPal cobre en producción y no en sandbox.)*
