...
- **Estado (✅ frontend desplegado y funcionando):** flujo completo operativo, **incluido el drag-and-drop** (terminado y reescrito sobre Pointer Events) y las **tarjetas grandes con foto**. **Pendiente:** REDESPLEGAR la Cloud Function `getPublicGiftRegistry` por Cloud Shell — el mismo redeploy (a) corrige un bug por el que la wishlist salía vacía aunque el dueño tuviera productos y (b) hace llegar a la página los campos nuevos `recipientPhoto`/`roleKey`/`gender` (foto + ocasiones de las tarjetas). Detalle completo, bugs y decisiones abiertas en [PLAN-FECHAS-ESPECIALES.md → "ESTADO Y FUNCIONAMIENTO — Registro de regalos por fecha (/regalar)"](./PLAN-FECHAS-ESPECIALES.md).
- **Archivos clave:** `src/pages/GiftRegistry/GiftRegistryPage.jsx` (tarjetas grandes + hook `useGiftDrag` por Pointer Events), `functions/index.js` (`getPublicGiftRegistry`, expone `recipientPhoto`/`roleKey`/`gender`), `src/pages/cuenta/CuentaFechasImportantesPage.jsx` (subida de la foto, `recipient.photoUrl`), `src/contexts/CartContext.jsx`, `src/pages/CheckoutPage.jsx`, `src/services/wishlist.js`, `src/App.jsx`
