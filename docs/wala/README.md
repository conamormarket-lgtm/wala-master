...
| 1.6 | [PLAN-AVATAR-3D.md](./PLAN-AVATAR-3D.md) | Plan (POR IMPLEMENTAR) del **Avatar Studio 3D**: selfie → avatar 3D (anime/Meta/realista) vía servicio de IA externo → visor 3D girable "estilo Pacdora" → probador de ropa (try-on). Compara proveedores (Avaturn, RPM **en cierre**, Meshy/Tripo, VRoid…), arquitectura con Cloud Function + Storage, privacidad/PII de selfies, fases y costos. | Antes de tocar el Avatar Studio del perfil; **leer ya** por el cierre de Ready Player Me. |
| 1.7 | [PLAN-I18N.md](./PLAN-I18N.md) | Plan (POR IMPLEMENTAR) de **internacionalización**: detección de idioma/país (reusa `geo.js`), UI con `react-i18next` (es/en/pt, $0), popup "ver en tu idioma/original" + toggle en Header, y traducción de catálogo con **Google Cloud Translation v3 + caché Firestore**. Compara Google vs DeepL, costos y fases. | Antes de implementar i18n / atención a extranjeros. |
