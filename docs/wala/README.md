...
| 1.12 | [SORTEO-POR-SUSCRIPCION.md](./SORTEO-POR-SUSCRIPCION.md) | Módulo **Sorteo por suscripción** (estilo Jorge Luna, "No Hay Sin Suerte"): rifa con **auto-débito recurrente** (Culqi guarda `Customer`+`Card` y **cobra por CRON diario** `cobrarSuscripcionesCulqi` `0 9 * * *`; PayPal Subscriptions internacional con **webhook de firma verificada, FAIL-CLOSED**), **chances proporcionales a los meses del plan** (mensual×1…anual×12) y **regla dura: SOLO suscriptores `activo` + `vigenciaHasta>=ahora` ganan** (reusa el RNG cripto de `decidirGanadoresSorteo`). Público `/suscrito-sorteo` + `/suscrito-sorteo/:slug` (`SuscripcionSorteoPage` + `suscripcion/PagoSuscripcion`), admin `/admin/sorteos-suscripcion` + `/:id`, servicio `src/services/suscripcionSorteos.js`, modelo `sorteos_suscripcion/{id}` + subcolecciones y las **8 Cloud Functions** del módulo. **Frontend + CFs + reglas + índices DESPLEGADOS**; pendiente del dueño: **webhook de PayPal** + **crear la campaña**. **100 % aditivo al pago único.** | Antes de tocar suscripciones, el auto-débito recurrente o el reparto de ganadores por suscripción. |
