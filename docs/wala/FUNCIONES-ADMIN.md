...
| **Elementos con diseño** | `/admin/elementos-diseno` | Hub para **editar las piezas visuales de cada marca**. Eliges una marca y editas su **nav de categorías** (las burbujas): qué categoría filtra cada burbuja, su nombre, su miniatura, y agregar/quitar/reordenar. **Enlace propio en el menú lateral** (🎨 Elementos con diseño, **debajo de "Recepción de Pedidos"**). Estructura de **pestañas preparada para crecer** (más elementos por marca en el futuro). |
...
Es un panel **SOLO LECTURA del ERP** que muestra **todos los pedidos del portal WALA** (no los del usuario logueado). La capa de datos `adminOrders.getWalaOrdersForAdmin` lee las colecciones **`pedidos_web` (fuente principal) + `pedidos`** (ordenadas por fecha desc con límite), se queda solo con los pedidos del portal (regla **`esPedidoWala`**: `canalVenta:'Portal Web'` o los respaldos `web:true` / `activador:'portal_web'` / `vendedor:'Portal Web'`) y **deduplica por clave de negocio (`numeroPedido` → respaldo `portalPseudoOrderId`/`id`)**, porque un mismo pedido del portal puede existir a la vez en `pedidos_web` y, una vez validado, en `pedidos` con otro id (gana la versión de `pedidos_web`). Está orientado a **preparar y organizar los envíos**:
...
Archivo: `src/pages/admin/AdminElementosDiseno.jsx` (editor reutilizable: `src/components/admin/CategoryNavEditor/CategoryNavEditor.jsx`).
**Hub para editar las piezas visuales de cada marca.** La página tiene un **selector de marca** arriba (común a todas las pestañas) y un **sistema de pestañas preparado para crecer**: hoy trae **una** pestaña, **"Navegación por categorías"**, pero la estructura está pensada para sumar más "elementos con diseño" por marca en el futuro (banners, destacados, etc.) sin rehacer la pantalla. Enlace propio en el menú lateral (**🎨 Elementos con diseño**, **debajo de "Recepción de Pedidos"** en el grupo *Diseño de Tienda*); gateada por `AdminRoute` + `canDesign` (Super Admin o permiso *Diseño de Tienda*).
...
> **Regla clave:** si el `categoryNav` de la marca **tiene burbujas**, esas **reemplazan** al nav automático; si está **vacío**, manda el **nav automático** (derivado de los productos). Como el nav vive en la **marca**, lo que edites aquí se refleja en **todas las páginas** que usen ese nav.

> **Dos caminos para lo MISMO:** este editor (`CategoryNavEditor`) es el **mismo** que aparece (a) en *Marcas → "Gestionar productos" → pestaña "Nav de categorías"* y (b) **inline en el Editor Visual** al seleccionar una sección "Navegación por categorías" con marca. Los tres editan el `categoryNav` de la marca, así que los cambios están **sincronizados**.
...
- **Nav de categorías** — **OVERRIDE OPCIONAL**: por defecto el módulo "Navegación por categorías" de la marca **NO necesita esta pestaña**, porque las burbujas se generan **automáticamente** desde las categorías de los productos de la marca (ver más abajo, "Selector de MARCA por sección"). Esta pestaña monta el **mismo editor reutilizable `CategoryNavEditor`** que la sección *Elementos con diseño* (`/admin/elementos-diseno`): por cada burbuja edita **qué categoría filtra** (`categoryId`), su **nombre** y su **miniatura** (subir+recortar 1:1 o **heredar** de la categoría), además de **agregar/quitar/reordenar**, **"Generar automático"** (prellena con las categorías de los productos de la marca) y **"Vaciar (volver a automático)"**. Guarda el array **`categoryNav`** (`[{ categoryId, name, imageUrl, order }]`) con `updateBrand(brand.id, { categoryNav })`, embebido en `tienda_brands`. **Si este array tiene items, REEMPLAZA a las burbujas automáticas**; si está vacío, manda el nav automático. *(Como el `categoryNav` vive en la marca, esta pestaña, "Elementos con diseño" y el inline del Editor Visual editan lo mismo y quedan sincronizados.)*
...
- En `categories_nav`, las burbujas son **AUTOMÁTICAS**: si la marca **no** tiene un `categoryNav` manual, `TiendaPage` **deriva las categorías de los PRODUCTOS de la marca** (`getProductsByBrand` → categorías distintas, con la **misma extracción de id** que el filtro del sidebar) y las **mapea a `tienda_categories`** (`getCategories`) para sacar **nombre + imagen**. Las **imágenes salen de `/admin/categorías`** (`tienda_categories.imageUrl`); **una categoría sin imagen = burbuja con la inicial + un color estable** (ya no se usan fotos de stock). Al pulsar una burbuja se **fija la categoría** (estado **compartido** con el sidebar de la misma página → filtra el catálogo **sin navegar**). El `categoryNav` manual sigue como **OVERRIDE opcional**.
  - **Editor inline del nav (commit `fc8a0d2`):** cuando la sección "Navegación por categorías" tiene una **marca seleccionada**, el Editor Visual **embebe inline el mismo `CategoryNavEditor`** que la sección *Elementos con diseño* y el panel por marca. Así el dueño edita las burbujas (qué filtra, nombre, miniatura, orden, generar/vaciar) **sin salir del editor**. Como el `categoryNav` **vive en la marca**, esos cambios se **sincronizan** con los otros dos lugares y afectan a **todas las páginas** que usen ese nav. Sin marca, las burbujas son automáticas.
...
- **Archivos clave:** `src/pages/Tienda/admin/VisualEditorPanel.jsx` (módulos por marca en "Añadir" + selector de marca + **inline `CategoryNavEditor`**), `src/pages/admin/AdminElementosDiseno.jsx` (sección "Elementos con diseño"), `src/components/admin/CategoryNavEditor/CategoryNavEditor.jsx` (editor reutilizable del nav, embebido en los 3 lugares), `src/pages/admin/AdminMarcaProductos.jsx` (pestaña "Nav de categorías"), `src/pages/Tienda/services/storefront.js` (defaults `brandId:''`; label "Catálogo (todas las marcas)"), `src/pages/Tienda/TiendaPage.jsx` (nav AUTO-derivado + `pageBrandId` por nav), `src/pages/Tienda/components/VisualCategoryNav/VisualCategoryNav.jsx` (burbuja con inicial cuando no hay imagen), `src/services/products.js` (`getProductsByBrand`, faceta `brand`), `src/services/brands.js` (`slug`, `categoryNav`), `scripts/setup-marcas.js`.
...
| **Navegación por categorías** | Fila/burbujas de accesos a categorías. **Con setting `brandId`** (multi-marca): las burbujas son **AUTOMÁTICAS** — se derivan de las categorías que tienen los **productos de esa marca**, con la imagen de cada categoría (de `/admin/categorías`; sin imagen = burbuja con inicial). Al pulsarlas, **filtra el catálogo de la misma página** (no navega a `/tienda?categoria=`). Un `categoryNav` manual en el panel de la marca actúa como **override**. Sin `brandId`, es navegación global por enlaces. *Se inserta listo desde "Añadir Nuevo Módulo" → "Categorías {Marca}".* |
