...
const EditorCanvas = ({ productImage, printAreas = [], viewId, showZones = true }) => {
  ...
  const getContainerDimensions = () => {
    ...
    return {
      w: el?.offsetWidth || DEFAULT_CANVAS_WIDTH,
      h: el?.offsetHeight || DEFAULT_CANVAS_HEIGHT
    };
  };
  ...
  React.useLayoutEffect(() => {
    ...
    const updateScale = () => {
      ...
      const scale = w > 0 ? Math.min(1, w / imageDimensions.totalW) : 1;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const printAreasToRender = Array.isArray(printAreas)
        ? printAreas
        : [{ id: 'default', shape: SHAPE_TYPES.RECTANGLE, x: 10, y: 10, width: 80, height: 80 }];
  ...
  useEffect(() => {
    ...
    try {
      const printAreasToRender = Array.isArray(printAreas)
        ? printAreas
        : [{ id: 'default', shape: SHAPE_TYPES.RECTANGLE, x: 10, y: 10, width: 40, height: 40 }];
  ...
  const innerContent = (
    <div
      ...
      style={{
        ...
        height: scaledH,
        position: 'relative',
        overflow: 'visible', // Permitir que las zonas sobresalgan un poco
      }}
    >
