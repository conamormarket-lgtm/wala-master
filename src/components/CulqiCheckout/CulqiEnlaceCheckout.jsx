...
const CulqiEnlaceCheckout = ({ enlace, onSuccess }) => {
  const [error, setError] = useState(null);
  const [isProcessing, setIsProcessing] = useState(false);
  const [isCulqiLoaded, setIsCulqiLoaded] = useState(false);

  const amountInUSD = Number(enlace?.montoUSD || 0);
  ...
  const abrirCulqi = () => {
    ...
    if (amountCents < 300) {
      setError("⚠️ Culqi requiere un monto mínimo de S/ 3.00 para procesar tarjetas por políticas bancarias. Por favor, crea un nuevo enlace con al menos 3.00 para probar.");
      return;
    }

    window.Culqi.publicKey = publicKey;
    window.Culqi.settings({
      title: 'Walá Pago Rápido',
      currency: 'PEN',
      amount: amountCents,
    });
  ...
  return (
    <div style={{ marginTop: '1.5rem', width: '100%' }}>
      ...
      <div style={{ display: isProcessing ? 'none' : 'flex', justifyContent: 'center' }}>
        <button 
          onClick={abrirCulqi}
      ...
      <div style={{ display: isProcessing ? 'none' : 'flex', justifyContent: 'center', gap: '16px', marginTop: '16px', opacity: 0.7 }}>
        {/* Visa */}
        <svg viewBox="0 0 38 12" height="20" fill="#1434CB">
        ...
        {/* Amex */}
        <svg viewBox="0 0 32 20" height="20" fill="#006FCF">
      ...
      {isProcessing && !error && (
        <div style={{ textAlign: 'center', color: '#7C3AED', padding: '1.5rem', background: '#F5F3FF', borderRadius: '8px' }}>
...
export default CulqiEnlaceCheckout;
